...
if(SGX_FOUND)
    ...
    if(BUILD_TYPE STREQUAL "Deploy")
        message(STATUS "Deploy mode: SGX_HW=ON, SGX_MODE=Release, BUILD_TESTS=OFF")
        set(SGX_HW ON)
        set(SGX_MODE "Release")
        set(BUILD_TESTS OFF)
    else()
        message(STATUS "Develop mode: SGX_HW=OFF, SGX_MODE=Debug, BUILD_TESTS=ON")
        set(SGX_HW OFF)
        set(SGX_MODE "Debug")
        set(BUILD_TESTS ON)
    endif()
    ...
    if(BUILD_TESTS)
        message(STATUS "Building unit and integration test suite")
    else()
        message(STATUS "Not building unit and integration test suite, use BUILD_TESTS=ON to build")
    endif()

    if(SGX_HW)
        message(STATUS "Building in hardware mode (requires SGX PSW and drivers)")
    ...
    elseif(SGX_MODE STREQUAL "PreRelease")
        message(STATUS "Building in pre-release mode")
        set(SGX_COMMON_CFLAGS "${SGX_COMMON_CFLAGS} -O2 -UDEBUG -DNDEBUG -DEDEBUG")
    elseif(SGX_MODE STREQUAL "Release")
        message(STATUS "Building in release mode")
        set(SGX_COMMON_CFLAGS "${SGX_COMMON_CFLAGS} -O2 -UDEBUG -DNDEBUG -UEDEBUG -DPRODUCTION_IAS")
    else()
    ...
    set(ENCLAVE_COMMON_FLAGS "${SGX_COMMON_CFLAGS} ${SGX_WARNING_FLAGS} -nostdinc -fvisibility=hidden -fpie -fstack-protector-strong -ffunction-sections -fdata-sections")
    set(ENCLAVE_C_FLAGS "${ENCLAVE_COMMON_FLAGS} ${SGX_WARNING_CFLAGS}")
    set(ENCLAVE_CXX_FLAGS "${ENCLAVE_COMMON_FLAGS} ${SGX_WARNING_CXXFLAGS} -nostdinc++")
    ...
    function(add_trusted_library target)
        ...
        target_link_libraries(${target} "${SGX_COMMON_CFLAGS} ${SGX_SECURITY_FLAGS} \
            -Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L${SGX_LIBRARY_PATH} ${EXTRA_LIB_PATHS} \
            -Wl,--start-group ${EXTRA_LIBS} -lsgx_tstdc -lsgx_tcxx -lsgx_tkey_exchange -lsgx_tcrypto -l${SGX_TSVC_LIB} -Wl,--end-group \
    ...
    function(add_enclave_library target)
        ...
        target_link_libraries(${target} "${SGX_COMMON_CFLAGS} ${SGX_SECURITY_FLAGS}\
            ...
            -Wl,--start-group ${TLIB_LIST} ${EXTRA_LIBS} -lsgx_tstdc -lsgx_tcxx -lsgx_tkey_exchange -lsgx_tcrypto -l${SGX_TSVC_LIB} -Wl,--end-group \
            -Wl,-Bstatic -Wl,-Bsymbolic -Wl,--no-undefined \
            -Wl,-pie,-eenclave_entry -Wl,--export-dynamic \
            ${LDSCRIPT_FLAG} \
            -Wl,--defsym,__ImageBase=0")
